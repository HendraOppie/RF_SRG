...
#Region "Private"
    ...
    Private Sub p_Menu()
        eActiveUI = RFUiList.RECEIVING_00
    ...
    Private Sub p_Main(Optional sMessage As String = "", Optional bError As Boolean = False)
        Try
            ...
            PrintLine(1, 4, "ASN#    :")
            PrintLine(LEFT_INPUT_POS, 4, dtScreen.ReceiptKey)
    ...
    Private Sub p_printList(iTop As Int16)
        ...
        If dtScreen.alDataAsnSummary.Count = 0 Then Exit Sub
        For Each dtAsnSumary As Receipt.DataReceiptDetail In dtScreen.alDataAsnSummary
            If iTop = MAX_TOP_PRINTLIST Then Exit For

            Dim dtSumSkuScanned As ReceivingScan.DataReceivingScan = p_getDataScanFromList(dtAsnSumary.Sku)
            Dim balanceQty As Int16 = dtAsnSumary.QtyExpected - dtSumSkuScanned.Qty

            If balanceQty > 0 Then
                PrintLine(1, iTop, String.Format("{0} Q{1}", dtAsnSumary.Sku, balanceQty.ToString()))
                iTop += 1
                bolHasBalance = True
            End If
        Next
    ...
    Private Function p_getDataScanFromList(Sku As String) As ReceivingScan.DataReceivingScan
        Dim dtSumSkuScanned As New ReceivingScan.DataReceivingScan

        For Each dtScanSummary As ReceivingScan.DataReceivingScan In dtScreen.alDataSummary
    ...
    Private Sub p_scanReceiptKey()
        Try
            If dtScreen.ReceiptKey = String.Empty Then
                elapseStart = Now()
                p_getAsn()
                p_getList()
                p_Main()
    ...
    Private Sub p_getList()
        Try
            If dtScreen.ReceiptKey <> String.Empty Then
                dtScreen.alData = New RF.ReceivingScan().GetList(dtScreen.ReceiptKey, dtScreen.StorerKey, dtConnInfo)
                dtScreen.alDataSummary = New RF.ReceivingScan().GetSummarySku(dtScreen.ReceiptKey, dtScreen.StorerKey, dtConnInfo)
            End If

            For Each dtSumScanned As ReceivingScan.DataReceivingScan In dtScreen.alDataSummary
    ...
    Private Sub p_getAsn()
        Try
            dtScreen.ReceiptKey = GetScanFixEnterPrint(LEFT_INPUT_POS, 4, False, 1)
            If dtScreen.ReceiptKey = String.Empty Then
            ...
            dtScreen.alDataAsnSummary = New Receipt().GetSummarySku(dtScreen.ReceiptKey, dtConnInfoWms)
            If dtScreen.alDataAsnSummary.Count = 0 Then
                dtScreen = New DataScreen
                p_Main("ASN NOT FOUND", True)
            End If

            For Each dtAsnSum As Receipt.DataReceiptDetail In dtScreen.alDataAsnSummary
                dtScreen.Total += dtAsnSum.QtyExpected
                dtScreen.StorerKey = dtAsnSum.StorerKey
            Next
        Catch ex As Exception
            Throw
        End Try
    ...
    Private Sub p_save()
        Dim dtScanned As New ReceivingScan.DataReceivingScan
        dtScanned.QRCode = dtScreen.QrCode
        dtScanned.ReceiptKey = dtScreen.ReceiptKey
        dtScanned.ExternReceiptKey = dtScreen.ExternReceiptKey
        ...
        Dim oReceivingScan As New ReceivingScan
        oReceivingScan.Save(dtScanned, dtConnInfo)
    End Sub

    Private Sub p_validateExistsScanInList()
        Dim bFound As Boolean = False
        Dim qtyWms As Int16 = 0
        For Each dtAsnWms As Receipt.DataReceiptDetail In dtScreen.alDataAsnSummary
            If dtAsnWms.Sku = dtScreen.Sku Then
                bFound = True
                qtyWms = dtAsnWms.QtyExpected
                Exit For
            End If
        Next
    ...
    Private Sub p_validateDuplicateScanInList()
        Dim bFound As Boolean = False
        For Each dtReceivingScan As ReceivingScan.DataReceivingScan In dtScreen.alData
            If dtScreen.QrCode = dtReceivingScan.QRCode Then
                bFound = True
                Exit For
            End If
        Next
...
#Region "Data Class"
    ...
    Public Class DataScreen
        ...
        Public ExternReceiptKey As String = String.Empty
        ...
        Public alDataAsnSummary As New ArrayList
    End Class
